...
let data =  getData('');//require('../../Backend/app/data/2019-05-03/amc-columbus-10.json');
...
function getShowtimes(theatre, theatreInfo) {
    ...
    for (movie of theatreInfo) {
        for (showtime of movie.times) {
            showtimes.push({
                title: movie.title,
...
function main() {
    showtimes = getShowtimes('AMC Columbus', data).sort((s1, s2) => {
